...
from num2words_inr import rupees_in_words, number_to_words_inr
import datetime
...
def _name_designation_office(case, sep=', '):
    """'Sh./Smt. NAME, DESIGNATION, OFFICE' - but skips any of designation/office
    that are blank, instead of leaving stray double commas like 'NAME, , ,'."""
    parts = [case.get('designation', ''), case.get('office', '')]
    parts = [p.strip() for p in parts if p and p.strip()]
    tail = sep.join(parts)
    if tail:
        return f"Sh./Smt. {case['name']}{sep}{tail}"
    return f"Sh./Smt. {case['name']}"
...
def build_sanction_memo(case, out_path):
    ...
    """
    amount_words = case.get('amount_words') or (number_to_words_inr(case['amount']) + ' Only')
    doc = Document()
    ...
    body_text = (
        ...
        f"(Rs. {amount_words}) to {_name_designation_office(case)}, "
        f"Pin – {case['pin']}, Emp. ID –{case['emp_id']} being the expenses incurred by him/her "
    ...
    copy_list = [
        f"1- {_name_designation_office(case)}, Pin - {case['pin']}",
...
def build_ns_checklist(case, items, out_path):
    ...
    case: same dict as above, plus:
      total_claimed (the actual bill total - can differ from case['amount'], which is
        whatever is being sanctioned), total_admissible, calc_sheet_sl_no (reference
        number for the appended calculation-sheet annexure), tier_label,
      cghs_card_no, cghs_validity, submission_date, hco_type ('Govt/CGHS empanelled/Private'),
      nabh_status ('NABH'/'Non-NABH'/'Super Speciality'), treatment_type ('OPD'/'Indoor'/'TEST/INVESTIGATION'/'Emergency')
    items: list of dicts with 'sl_no','particulars','code','claimed','admissible' - these
      populate the Calculation Sheet annexure appended at the end (referenced by Sl. No.
      from the summary table), matching how this division actually files item-wise workings
      as a separate sheet rather than inline in the main consolidation table.
    """
    doc = Document()
    style = doc.styles['Normal']
    style.font.name = 'Calibri'
    ...
    amount = case['amount']
    bill_amount = case.get('total_claimed', amount)
    bill_amount_words = case.get('total_claimed_words') or (number_to_words_inr(bill_amount) + ' Only')

    _add_para(doc, case['memo_no'], size=10, align=WD_ALIGN_PARAGRAPH.LEFT, space_after=8)

    intro = (
        f"This is regarding sanction of medical bill of Rs. {bill_amount}/- (Rs. {bill_amount_words}) "
        f"incurred by {_name_designation_office(case)}, "
    ...
    calc_sl_no = case.get('calc_sheet_sl_no', '05')

    table = doc.add_table(rows=1, cols=6)
    table.alignment = WD_TABLE_ALIGNMENT.CENTER
    headers = ['Sl. No.', 'Particulars', 'Code', 'Amount\nclaimed', 'Amount\nreimbursable',
               'Calculation sheet\nis placed at Sl. No.']
    ...
    # A single summary row, matching the division's actual format: the item-wise
    # workings live in a separately-referenced calculation sheet, not spelled out here.
    row = table.add_row().cells
    row[0].text = ''
    row[1].text = ''
    row[2].text = ''
    row[3].text = f"{bill_amount}/-"
    row[4].text = f"{case.get('total_admissible', amount)}/-"
    row[5].text = str(calc_sl_no)
    for c in row:
    ...
    total_line = (
        ...
        f"(Rs. {case.get('total_admissible_words') or number_to_words_inr(case.get('total_admissible', amount))} Only). "
    ...
    checklist_rows = [
        ("1. (i)", "Name & Designation and office of the official", f"{_name_designation_office(case)}, Pin – {case['pin']}, Emp Id- {case['emp_id']}"),
        ...
        ("14.", "Total amount claimed by the official", f"{case.get('total_claimed', amount)}/"),
    ...
    # --- Calculation Sheet annexure - the item-wise CGHS rate check, referenced
    # from the summary table above by Sl. No., exactly as this division files it. ---
    if items:
        doc.add_page_break()
        _add_para(doc, f"Calculation Sheet (Sl. No. {calc_sl_no})", size=13, bold=True,
                   align=WD_ALIGN_PARAGRAPH.CENTER, space_after=4)
        _add_para(doc, "Item-wise CGHS rate verification", size=10.5,
                   align=WD_ALIGN_PARAGRAPH.CENTER, space_after=14)

        ctable2 = doc.add_table(rows=1, cols=5)
        ctable2.alignment = WD_TABLE_ALIGNMENT.CENTER
        headers2 = ['Sl. No.', 'Particulars', 'CGHS Code', 'Amount claimed', 'Amount admissible']
        hdr2 = ctable2.rows[0].cells
        for i, h in enumerate(headers2):
            ...
            for p in hdr2[i].paragraphs:
                for r in p.runs:
                    r.font.bold = True
                    r.font.size = Pt(10)
            _set_cell_border(hdr2[i])

        for it in items:
            row = ctable2.add_row().cells
            row[0].text = str(it.get('sl_no', ''))
            row[1].text = it.get('particulars', '')
            row[2].text = str(it.get('code') or '-')
            row[3].text = f"{it.get('claimed', '-')}/-"
            admissible_val = it.get('admissible', '-')
            row[4].text = f"{admissible_val}/-" if admissible_val != 'MANUAL REVIEW' else 'MANUAL REVIEW'
            for c in row:
                for p in c.paragraphs:
                    for r in p.runs:
                        r.font.size = Pt(9.5)
        ...
        _add_para(
            doc,
            f"Total claimed: Rs. {case.get('total_claimed', amount)}/-  |  Total admissible as per CGHS rates "
            f"({case.get('tier_label','-')}, {case.get('nabh_status','-')}): "
            f"Rs. {case.get('total_admissible', amount)}/-",
            size=10.5, bold=True, space_after=0
        )
